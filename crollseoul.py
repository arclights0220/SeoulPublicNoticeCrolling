...
def gangseo(a):
    try:
        ...
        iframe = driver.find_element_by_xpath(
            "/html/body/div[2]/main/div[2]/div[1]/div/iframe")
        driver.switch_to.frame(iframe)
...
def nowon(a):
    try:
        ...
        driver.get(url)
        box = driver.find_element_by_xpath('/html/body/div/div/article/div[2]/div[2]/form/fieldset/div[1]/input')
        btn = driver.find_element_by_xpath('/html/body/div/div/article/div[2]/div[2]/form/fieldset/div[2]/span[1]/button')
        box.send_keys(a)
        btn.click()
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
...
def dongjak(a):
    try:
        ...
        soup = BeautifulSoup(html, 'html.parser')
        search_result = soup.select_one(
            '#wrapper > div.sub-container > div > div.content-area > div.nw-content-data > div > div.bdList > table > tbody > tr:nth-child(1) > td.title > a')
        search_result_date = soup.select_one(
            "#wrapper > div.sub-container > div > div.content-area > div.nw-content-data > div > div.bdList > table > tbody > tr:nth-child(1) > td:nth-child(4)"
        )
        if not search_result:
            print("\n동작구청 에러")
            webbrowser.open(url)
            ...
            onlytext = search_result.getText()
            splittext = onlytext.split()
            onlytext2 = search_result_date.getText()
            splitext2 = onlytext2.split()
            print("\n동작구청 " + a + "에 관한 최신공고 : " + ' '.join(str(_)
                                                            for _ in splittext) + " " + ' '.join(str(_)for _ in splitext2))
    except:
        print("\n동작구청 오류")
        webbrowser.open(url)
        print("\n", url)
...
def junggu(a):
    try:
        ...
        driver.get(url)
        box = driver.find_element_by_xpath('/html/body/div/div[3]/div[2]/div[2]/div[2]/form/div/div/input[1]')
        btn = driver.find_element_by_xpath(
            '/html/body/div/div[3]/div[2]/div[2]/div[2]/form/div/div/input[3]')
        box.send_keys(a)
        btn.click()
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
...
def seocho(a):
    try:
        ...
        driver.get(url)
        iframe = driver.find_element_by_xpath(
            "/html/body/div[1]/div[3]/div/div[2]/div[2]/div/iframe")
        driver.switch_to.frame(iframe)
        box = driver.find_element_by_id("temp")
...
def sb(a):
    try:
        ...
        btn = driver.find_element_by_xpath(
            '/html/body/div[3]/div[3]/div[2]/div/section/div[1]/article/form/fieldset/input[1]')
        box.send_keys(a)
        btn.click()
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
...
def songpa(a):
    try:
        ...
        driver.get(url)
        box = driver.find_element_by_xpath('/html/body/div/div/div/main/article/div/div/div[1]/form/fieldset/div/div/input[3]')
...
def ydp(a):
    try:
        ...
        url = "https://www.ydp.go.kr/www/selectEminwonList.do?menuFlag=01&key=2851"
        driver.get(url)
        box = driver.find_element_by_xpath(
...
def jongno(a):
    try:
        ...
        select = Select(driver.find_element_by_xpath('/html/body/div[4]/div[2]/div/div[2]/div/div[2]/form/fieldset/div/div/select'))
        select.select_by_visible_text('제목')
        box.send_keys(a)
        btn.click()
        html = driver.page_source
...
def junggu(a):
    try:
        ...
        driver.get(url)
        box = driver.find_element_by_xpath('/html/body/div/div[3]/div[2]/div[2]/div[2]/form/div/div/input[1]')
        btn = driver.find_element_by_xpath("/html/body/div/div[3]/div[2]/div[2]/div[2]/form/div/div/input[3]")
        box.send_keys(a)
        btn.click()
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
